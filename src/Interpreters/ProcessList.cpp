...
ProcessList::insert(const String & query_, const IAST * ast, ContextMutablePtr query_context, UInt64 watch_start_nanoseconds)
{
...

    {
        ...
        CancellationChecker::getInstance().appendTask(query, query_context->getSettingsRef().max_execution_time.totalMilliseconds(), query_context->getSettingsRef().timeout_overflow_mode);
...
void QueryStatus::addPipelineExecutor(PipelineExecutor * e)
{
    ...
    /// lead to deadlock.
    UInt64 max_exec_time = getContext()->getSettingsRef().max_execution_time.totalMilliseconds();
