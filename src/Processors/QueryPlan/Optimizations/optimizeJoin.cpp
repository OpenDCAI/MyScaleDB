...
bool optimizeJoinLegacy(QueryPlan::Node & node, QueryPlan::Nodes &, const QueryPlanOptimizationSettings &)
{
    ...
    if (join_step->swap_join_tables_mode)
    {
    ...
    // const auto & headers = join_step->getInputHeader();
    const auto & streams = join_step->getInputStreams();
    if (streams.size() != 2)
        return true;
    
    const auto & left_stream_input_header = streams[0].header;
    const auto & right_stream_input_header = streams[1].header;

    auto updated_table_join = std::make_shared<TableJoin>(table_join);
    [[maybe_unused]] const auto update_storage = updated_table_join->getStorageJoin();

    [[maybe_unused]] const auto storage = updated_table_join->getStorageJoin();
...
{
    const auto & stream = node->step->getOutputStream();

    QueryPlanStepPtr step;

    if (filter_column_name.empty())
        step = std::make_unique<ExpressionStep>(stream, std::move(actions_dag));
    else
        step = std::make_unique<FilterStep>(stream, std::move(actions_dag), filter_column_name, false);
    ...
    const JoinInfo & join_info)
{
    ...
    auto add_sorting = [&] (QueryPlan::Node *& node, const Names & key_names, JoinTableSide join_table_side)
    {
        ...
        auto sorting_step = std::make_unique<SortingStep>(
            node->step->getOutputStream(), std::move(sort_description), 0 /*limit*/, sort_settings, true /*is_sorting_for_merge_join*/);
    ...
    auto add_create_set = [&](QueryPlan::Node *& node, const Names & key_names, JoinTableSide join_table_side)
    {
        auto creating_set_step = std::make_unique<CreateSetAndFilterOnTheFlyStep>(
            node->step->getOutputStream(), key_names, join_settings.max_rows_in_set_to_optimize_join, crosswise_connection, join_table_side);
    ...
    bool has_non_const_keys = has_non_const(left_node->step->getOutputStream().header, join_clause.key_names_left)
        && has_non_const(right_node->step->getOutputStream().header , join_clause.key_names_right);
...
bool convertLogicalJoinToPhysical(QueryPlan::Node & node, QueryPlan::Nodes & nodes, const QueryPlanOptimizationSettings & optimization_settings)
{
    ...
    Header output_header = join_step->getOutputStream().header;
    ...
    if (!join_ptr->isFilled())
    {
        ...
        new_join_node.step = std::make_unique<JoinStep>(
            new_left_node->step->getOutputStream(),
            new_right_node->step->getOutputStream(),
            join_ptr,
    ...
    if (!post_filter)
        node.step = std::make_unique<ExpressionStep>(new_join_node.step->getOutputStream(), std::move(join_expression_actions.post_join_actions));
    else
    {
        bool remove_filter = !output_header.has(post_filter.column_name);
        node.step = std::make_unique<FilterStep>(new_join_node.step->getOutputStream(), std::move(join_expression_actions.post_join_actions), post_filter.column_name, remove_filter);
    }
...
bool optimizeJoinLogical(QueryPlan::Node & node, QueryPlan::Nodes &, const QueryPlanOptimizationSettings & optimization_settings)
{
    ...
    bool need_swap = false;
    if (optimization_settings.join_swap_table)
    {
        auto lhs_extimation = estimateReadRowsCount(*node.children[0]);
        auto rhs_extimation = estimateReadRowsCount(*node.children[1]);
