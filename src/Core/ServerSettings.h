...
#define SERVER_SETTINGS(M, ALIAS) \
    ...
    M(String, concurrent_threads_scheduler, "round_robin", "The policy on how to perform a scheduling of CPU slots in Concurrency Control. Algorithm used to govern how limited number `concurrent_threads_soft_limit` of CPU slots are distributed among concurrent queries. Scheduler may be changed at runtime without server restart.", 0) \
    \
    ...
    M(String, series_keeper_path, "/clickhouse/series", R"(Path in Keeper with auto-incremental numbers, generated by the `generateSerialID` function. Each series will be a node under this path.)", 0) \
    ...
    M(Bool, throw_on_unknown_workload, false, "Defines behaviour on access to unknown WORKLOAD with query setting 'workload'.", 0) \
    M(Bool, cpu_slot_preemption, false, "Defines how workload scheduling for CPU resources (MASTER THREAD and WORKER THREAD) is done.", 0) \
    M(UInt64, cpu_slot_quantum_ns, 10000000, "It defines how many CPU nanoseconds a thread is allowed to consume after acquired a CPU slot and before it should request another CPU slot. Makes sense only if `cpu_slot_preemption` is enabled and CPU resource is defined for MASTER THREAD or WORKER THREAD.", 0) \
    M(UInt64, cpu_slot_preemption_timeout_ms, 1000, "It defines how many milliseconds could a worker thread wait during preemption, i.e. while waiting for another CPU slot to be granted. After this timeout, if thread was unable to acquire a new CPU slot it will exit and the query is scaled down to a lower number of concurrently executing threads dynamically. Note that master thread never downscaled, but could be preempted indefinitely. Makes sense only when `cpu_slot_preemption` is enabled and CPU resource is defined for WORKER THREAD.", 0) \
