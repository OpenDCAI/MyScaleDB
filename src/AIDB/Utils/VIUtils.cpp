...
#include <AIDB/Common/MergedPartNameAndId.h>
    ...
    DB::MergeTreeMutableDataPartPtr new_data_part)
{
    ...
    DB::MergeTreeDataPartChecksums old_index_checksums = AIDB::getVectorIndexChecksums(old_data_part->getDataPartStoragePtr(), vec_index_name);
...
{
    String checksums_filename = AIDB::getVectorIndexChecksumsFileName(index_name);
...
getMergedSourcePartsFromFileName(const String & index_name, const DB::MergeTreeDataPartChecksums vector_index_checksums)
{
    ...
    String description_file_name = AIDB::getVectorIndexDescriptionFileName(index_name);
