...
#include <AIDB/Common/ScanThreadLimiter.h>
...
template <>
AIDB::Float32VectorDatasetPtr MergeTreeVSManager::generateVectorDataset(bool is_batch, const VSDescription & desc)
{
    ...
    if (is_batch)
    {
        ...
        return std::make_shared<AIDB::VectorDataset<Search::DataType::FloatVector>>(
                   query_vector_num,
...
template <>
AIDB::BinaryVectorDatasetPtr MergeTreeVSManager::generateVectorDataset(bool is_batch, const VSDescription & desc)
{
    auto & query_column = desc.query_column;
    if (!query_column)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Wrong query column type");
    ...
    if (is_batch)
    {
        ...
        return std::make_shared<AIDB::VectorDataset<Search::DataType::BinaryVector>>(
                query_vector_num,
                static_cast<int32_t>(dim),
    ...
    const Search::DenseBitmapPtr filter)
{
    ...
    const String search_index_name = vector_scan_desc.search_index_name; /// Support multiple indices on a vector column

    VectorScanResultPtr tmp_vector_scan_result = std::make_shared<CommonSearchResult>();

    AIDB::VectorDatasetVariantPtr vec_data;
    ...
    AIDB::VIParameter search_params = AIDB::convertPocoJsonToMap(vector_scan_desc.vector_parameters);
    ...
    int k = vector_scan_desc.topk > 0 ? vector_scan_desc.topk : AIDB::DEFAULT_TOPK;
    ...
    try
    {   
        auto vi_executor = data_part->segments_mgr->getSegment(search_index_name);
        if (!vi_executor)
            throw Exception(ErrorCodes::VECTOR_INDEX_CACHE_LOAD_ERROR, "Vector index {} not found in part {}", search_index_name, data_part->name);
    ...
    {
        AIDB::VIMetric metric = getVSMetric(data_part, vector_scan_desc);
    ...
    const VectorScanResultPtr & first_stage_vec_result)
{
    ...
    const String search_column_name = vector_scan_desc.search_column_name;
    const String search_index_name = vector_scan_desc.search_index_name;
    ...
    AIDB::VectorDatasetVariantPtr vec_data = generateVectorDataset<Search::DataType::FloatVector>(false, vector_scan_desc);
    ...
    auto vi_seg = data_part->segments_mgr->getSegment(search_index_name);
    ...
    {
        LOG_DEBUG(log_, "Index {} does not support two stage search in part {}, use first stage result directly", search_index_name, data_part->name);
...
VectorScanResultPtr MergeTreeVSManager::vectorScanWithoutIndex(
    ...
    const ReadRanges & read_ranges,
    const AIDB::VIBitmapPtr filter,
    AIDB::VectorDatasetPtr<T> & query_vector,
    const String & search_column,
    int dim,
    int k,
    bool is_batch,
    const AIDB::VIMetric & metric)
{
    ...
    static AIDB::LimiterSharedContext brute_force_context(getNumberOfPhysicalCPUCores() * 2);
    AIDB::ScanThreadLimiter limiter(brute_force_context, log);
    ...
    if (filter)
    {
        ...
        for (const auto & single_range : read_ranges)
        {
            ...
            while (current_mark < single_range.end_mark)
            {
                ...
                AIDB::VectorDatasetPtr<T> base_data;
                std::vector<typename AIDB::SearchIndexDataTypeMap<T>::VectorDatasetType, AllocatorWithMemoryTracking<typename AIDB::SearchIndexDataTypeMap<T>::VectorDatasetType>> vector_raw_data;
                ...
                if constexpr (T == Search::DataType::FloatVector)
                {
                    ...
                    base_data = std::make_shared<AIDB::VectorDataset<T>>(
                ...
                else if constexpr (T == Search::DataType::BinaryVector)
                {
                    ...
                    using BinaryVectorDatasetType = typename AIDB::SearchIndexDataTypeMap<Search::DataType::BinaryVector>::VectorDatasetType;
                ...
                AIDB::VIBitmapPtr row_exists = std::make_shared<AIDB::VIBitmap>(mark_left_rows, true);
    ...
    else
    {
        ...
        while (num_rows_read < total_rows_in_part)
        {
            ...
            if constexpr (T == Search::DataType::FloatVector)
            {
                ...
                vector_raw_data.assign(total_rows * dim, std::numeric_limits<typename AIDB::SearchIndexDataTypeMap<T>::VectorDatasetType>().max());
            ...
            AIDB::VIBitmapPtr row_exists = std::make_shared<AIDB::VIBitmap>(total_rows, true);
            ...
            base_data = std::make_shared<AIDB::VectorDataset<T>>(
                    ...
                    static_cast<int32_t>(dim),
                    const_cast<typename AIDB::SearchIndexDataTypeMap<T>::VectorDatasetType *>(vector_raw_data.data()));
    ...
    int delete_id_num)
{
    ...
    switch (T)
    {
        case Search::DataType::FloatVector:
        {
            ...
            else if (metric == AIDB::VIMetric::Cosine || metric == AIDB::VIMetric::L2)
            {
        ...
        case Search::DataType::BinaryVector:
        {
            if (metric == AIDB::VIMetric::Hamming || metric == AIDB::VIMetric::Jaccard)
            {
                per_distance = std::vector<float>(k * nq, std::numeric_limits<float>().max());
                if (delete_id_num > 0)
                    tmp_per_distance = std::vector<float>((k + delete_id_num) * nq, std::numeric_limits<float>().max());
    ...
    AIDB::SimpleSegment<T>::searchWithoutIndex(
    ...
    for (int q = 0; q < nq; q++)
    {
        ...
        for (int i = 0; i < k; i++)
        {
            if ((metric != AIDB::VIMetric::IP && final_distance[j] > per_distance[z])
                || (metric == AIDB::VIMetric::IP && final_distance[j] < per_distance[z]))
            {
