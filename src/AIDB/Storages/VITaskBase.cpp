...
#include <AIDB/Storages/VITaskBase.h>
...
{
    if (build_status.getStatus() == AIDB::SegmentBuiltStatus::SUCCESS && state != VITaskBase::State::NEED_FINALIZE)
        return static_cast<VITaskBase::State>(static_cast<int>(state) + 1);
    else if (build_status.getStatus() == AIDB::SegmentBuiltStatus::BUILD_RETRY)
        return state;
    else
        /// build error
...
void VITaskBase::recordBuildStatus()
{
    ...
    bool is_success
        = build_status.getStatus() == AIDB::SegmentBuiltStatus::SUCCESS || build_status.getStatus() == AIDB::SegmentBuiltStatus::BUILD_SKIPPED;
    if (ctx)
    {
    ...
    else
    {
        ...
        bool is_same = part->info.getPartNameWithoutMutation() == AIDB::cutMutVer(part_name);
        if (!is_same)
            return;
        vi_seg->getSegmentStatus()->setStatus(is_success ? AIDB::SegmentStatus::BUILT : AIDB::SegmentStatus::ERROR, build_status.err_msg);
    }
}
