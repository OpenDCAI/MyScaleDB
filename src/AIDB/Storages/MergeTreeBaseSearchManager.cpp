    ...
    LoggerPtr log)
{
    ...
    int top_k = vector_scan_desc.topk > 0 ? vector_scan_desc.topk : AIDB::DEFAULT_TOPK;
    ...
    return getTotalTopSearchResultImpl(
        vector_results, static_cast<UInt64>(top_k), desc_direction, log, HybridSearchFuncType::VECTOR_SCAN, vec_res_index);
}
    ...
    LoggerPtr log)
{
    ...
    return getTotalTopSearchResultImpl(text_results, static_cast<UInt64>(top_k), true, log, HybridSearchFuncType::TEXT_SEARCH);
}

ScoreWithPartIndexAndLabels MergeTreeBaseSearchManager::getTotalTopKSparseResult(
    const VectorAndTextResultInDataParts & sparse_results,
    const SparseSearchInfoPtr & sparse_search_info,
    LoggerPtr log)
{
    LOG_TRACE(log, "Total top k sparse results with size {}", sparse_search_info->topk);

    return getTotalTopSearchResultImpl(
        sparse_results, static_cast<UInt64>(sparse_search_info->topk), true, log, HybridSearchFuncType::SPARSE_SEARCH);
}
        ...
        LoggerPtr log)
{
    ...
    return getTotalTopSearchResultImpl(
        parts_with_vector_text_result, num_reorder, desc_direction, log, HybridSearchFuncType::VECTOR_SCAN, vec_res_index);
}
...
ScoreWithPartIndexAndLabels MergeTreeBaseSearchManager::getTotalTopSearchResultImpl(
    ...
    LoggerPtr log,
    const HybridSearchFuncType & hybrid_search_type,
    const size_t vec_res_index)
{
    ...
    for (const auto & mix_results_in_part : vector_text_results)
    {
        ...
        switch (hybrid_search_type)
        {
            ...
            case HybridSearchFuncType::SPARSE_SEARCH:
                ...
                break;
            default:
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Unsupported special search type to get total top-k result: {}", hybrid_search_type);
        }
        ...
        if (search_result && search_result->computed)
        {
            ...
            if (!label_col)
            {
                LOG_DEBUG(log, "getTotalTopKSearchResult: label column in {} result is null", getSearchFuncName(hybrid_search_type));
            }
            else if (!score_col)
            {
                LOG_DEBUG(log, "getTotalTopKSearchResult: score column in {} result is null", getSearchFuncName(hybrid_search_type));
            }
            else
            {
    ...
    LoggerPtr log)
{
    ...
    SparseSearchResultPtr sparse_result = mix_results.sparse_search_result;
    if (sparse_result && sparse_result->computed)
        getLabelsInSearchResult(label_ids, sparse_result, log);
    ...
    LoggerPtr log)
{
    ...
    if (label_ids.empty())
    {
        ...
        mix_results.sparse_search_result = nullptr;
    ...
    SparseSearchResultPtr sparse_result = mix_results.sparse_search_result;
    if (sparse_result && sparse_result->computed)
        mix_results.sparse_search_result = filterSearchResultByFinalLabels(sparse_result, label_ids, log);
}
