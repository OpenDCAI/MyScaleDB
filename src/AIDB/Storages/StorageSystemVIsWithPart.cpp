...
#include <AIDB/Common/SegmentInfo.h>
...
protected:
        ...
        MutableColumns & res_columns)
    {
        ...
        String part_relative_path = AIDB::getPartRelativePath(fs::path(part->getDataPartStorage().getFullPath()).parent_path());
        AIDB::CachedSegmentKey cache_key{part_relative_path, AIDB::cutMutVer(part->name), AIDB::cutMutVer(owner_part), index.name, index.column};
        ...
        if (column_mask[src_index++])
        {
            if (cached_indices.contains(cache_key.toString()))
                res_columns[res_index++]->insert(Search::enumToString(AIDB::SegmentStatus::LOADED));
            else if (vec_info)
                res_columns[res_index++]->insert(vec_info->statusString());
            else
                res_columns[res_index++]->insert(Search::enumToString(AIDB::SegmentStatus::PENDING));
        }
    ...
    Chunk generate() override
    {
        ...
        while (rows_count < max_block_size)
        {
            ...
            for (const auto & it : AIDB::VICacheManager::getAllItems())
