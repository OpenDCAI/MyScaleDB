...
/// Check if only select primary key column, _part_offset and vector/text/hybrid/sparse search functions.
...
MergeTreeReadTask::BlockAndProgress MergeTreeSelectWithHybridSearchProcessor::readFromPartWithHybridSearch()
{
    ...
    for (size_t ps = 0; ps < sample_block.columns(); ++ps)
    {
        ...
        LOG_DEBUG(log, "col_name in sample block {} in pos {}, rows: {}", col_name, ps, sample_block.rows());
...
void MergeTreeSelectWithHybridSearchProcessor::performFilterStep(Block & block)
{
    Blocks blocks;
    Block input_header = block.cloneEmpty();
    blocks.emplace_back(std::move(block));
    BlocksPtr blocks_ptr = std::make_shared<Blocks>(std::move(blocks));
    Pipe pipe(std::make_shared<BlocksSource>(blocks_ptr, std::move(input_header)));
    ...
    if (prewhere_info)
    {
        ...
        PullingPipelineExecutor pipeline_executor(pipeline);

        Blocks res_blocks;
        Block read_block;
        while (pipeline_executor.pull(read_block))
        {
            if (read_block)
                res_blocks.emplace_back(std::move(read_block));
        }

        block = concatenateBlocks(res_blocks);
    }
}
...
IMergeTreeSelectAlgorithm::BlockAndProgress MergeTreeSelectWithHybridSearchProcessor::readFromPartWithPrimaryKeyCache(bool & success)
{
    ...
    if (base_search_manager && base_search_manager->preComputed())
    {
        ...
        auto res_block = sample_block.cloneWithColumns(tmp_result_columns);

        /// add filter pipeline
        if (prewhere_info && can_skip_perform_prefilter)
        {
            LOG_TEST(log, "Before performFilterStep, res_block: {}, res_block rows: {}", res_block.dumpStructure(), res_block.rows());
            performFilterStep(res_block);
            LOG_TEST(log, "After performFilterStep, res_block: {}, res_block rows: {}", res_block.dumpStructure(), res_block.rows());
        }

        task->mark_ranges.clear();
        result_row_num = res_block.rows();
        if (result_row_num > 0)
        {
            MergeTreeReadTask::BlockAndProgress res = {res_block, result_row_num};
            return res;
        }
    ...
    size_t max_streams)
{
    ...
    {
        bool consider_rough_filter = false;
        if (context_->getSettingsRef().enable_rough_filter_setting)
        {
            size_t estimate_read_rows = data_part_->index_granularity.getRowsCountInRanges(mark_ranges);

            if ((estimate_read_rows * 1.0) / data_part_->rows_count > 0.5 && data_part_->rows_count > 1000000)
                consider_rough_filter = true;
        }
        ...
        auto [filter, rough_set_filter] = performPrefilter(mark_ranges, prewhere_info_copy, storage_, storage_snapshot_, data_part_,
                        block_size_params_, reader_settings_, context_, max_streams, consider_rough_filter);
        
        if (rough_set_filter)
        {
            for (const auto & range : mark_ranges)
            {
                size_t start_row = data_part_->index_granularity.getMarkStartingRow(range.begin);
                size_t end_row = data_part_->index_granularity.getMarkStartingRow(range.end);
                if (start_row == 0 && end_row == data_part_->rows_count)
                {
                    filter = nullptr;
                    break;
                }
                filter->batch_set(start_row, end_row);
            }
            can_skip_perform_prefilter = true;
        }

        LOG_TRACE(getLogger("executeSearch"), "can_skip_perform_prefilter = {}, rough_set_filter = {}", can_skip_perform_prefilter, rough_set_filter);
...
bool getFilterFromPipeline(Pipe & pipe, AIDB::VIBitmapPtr & filter, bool consider_rough_filter)
{
    QueryPipelineBuilder builder;
    builder.init(std::move(pipe));
    ...
    // check continuity of _part_offset column
    constexpr size_t SAMPLE_BLOCKS = 3; // scan first N blocks
    size_t blocks_analyzed = 0;
    size_t total_pairs = 0;
    size_t continuous_pairs = 0;
    bool strategy_decided = false;
    bool use_batch_strategy = false;
    ...
    while (filter_executor.pull(block))
    {
        ...
        // check continuity stage
        if (!strategy_decided && blocks_analyzed < SAMPLE_BLOCKS)
        {
            blocks_analyzed++;
            
            // calculate number of continuous pairs in current block
            size_t block_pairs = row_count > 1 ? row_count - 1 : 0;
            size_t block_continuous_pairs = 0;
            
            for (size_t i = 1; i < row_count; ++i)
            {
                if (col_data[i] == col_data[i-1] + 1)
                    block_continuous_pairs++;
            }
            
            total_pairs += block_pairs;
            continuous_pairs += block_continuous_pairs;
            
            // calculate continuity ratio so far
            double continuity_ratio = total_pairs > 0 ? 
                static_cast<double>(continuous_pairs) / total_pairs : 0.0;
            
            LOG_DEBUG(getLogger("getFilterFromPipeline"),
                     "Block {} continuity: {}/{} pairs (cumulative ratio: {:.2f})",
                     blocks_analyzed, 
                     block_continuous_pairs, block_pairs,
                     continuity_ratio);
            
            // if enough blocks have been analyzed or continuity is significantly high/low, determine strategy
            if (blocks_analyzed >= SAMPLE_BLOCKS || continuity_ratio > 0.75 || continuity_ratio < 0.25)
            {
                strategy_decided = true;
                use_batch_strategy = (continuity_ratio >= 0.5);
                
                LOG_DEBUG(getLogger("getFilterFromPipeline"),
                         "Strategy decided after {} blocks: continuity ratio={:.2f}, strategy={}",
                         blocks_analyzed,
                         continuity_ratio,
                         use_batch_strategy ? "batch" : "individual");
                
                // if continuity is high and rough filter is allowed, exit early
                if (use_batch_strategy && consider_rough_filter && continuity_ratio > 0.5)
                {
                    LOG_DEBUG(getLogger("getFilterFromPipeline"),
                             "Early exit with rough filter: high continuity detected ({:.2f})",
                             continuity_ratio);
                    return true; // exit early, not set filter
                }
            }
        }
        
        // apply the decided strategy to process data
        if (strategy_decided)
        {
            if (use_batch_strategy)
            {
                // batch set continuous IDs
                if (row_count > 0)
                {
                    UInt64 start_id = col_data[0];
                    UInt64 prev_id = start_id;
                    
                    for (size_t i = 1; i < row_count; ++i)
                    {
                        UInt64 curr_id = col_data[i];
                        
                        if (curr_id != prev_id + 1)
                        {
                            filter->batch_set(start_id, prev_id + 1);
                            start_id = curr_id;
                        }
        ...
        {
            // before strategy decided, process data with single set
            for (size_t i = 0; i < row_count; ++i)
            {
                filter->set(col_data[i]);
            }
...
std::pair<AIDB::VIBitmapPtr, bool> MergeTreeSelectWithHybridSearchProcessor::performPrefilter(
    MarkRanges mark_ranges,
    const PrewhereInfoPtr & prewhere_info_copy,
    const MergeTreeData & storage_,
    const StorageSnapshotPtr & storage_snapshot_,
    ...
    bool consider_rough_filter)
{
    ...
    else
    {
        ...
        rough_set_filter = getFilterFromPipeline<PullingPipelineExecutor>(pipe, filter, consider_rough_filter);
    }
    ...
    const MergeTreeReaderSettings & reader_settings_)
{
    ...
    auto process_part = [&](size_t part_index)
    {
        ...
        else if (query_info.sparse_search_info)
        {
            ...
            executeSearch(sparse_search_mgr, data, storage_snapshot_, data_part_,
                        block_size_params_, mark_ranges, prewhere_info_copy, reader_settings_,
                        context, num_streams);
