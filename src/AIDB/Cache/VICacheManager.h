...
#include <AIDB/Cache/CachedSegmentKey.h>
#include <AIDB/Cache/CachedSegment.h>
#include <AIDB/Storages/VIDescriptions.h>

namespace std
{
template <>
struct hash<AIDB::CachedSegmentKey>
{
    std::size_t operator()(AIDB::CachedSegmentKey const & key) const noexcept { return std::hash<std::string>{}(key.toString()); }
};
}
