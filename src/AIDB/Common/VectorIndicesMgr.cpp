            ...
            if (vi_seg)
            {
                ...
                else
                {
                    ...
                    for (const auto & cache_key : vi_seg->getCachedSegmentKeys())
                    {
                        ...
                        if (clear_cache)
                            AIDB::VICacheManager::removeFromCache(cache_key);
                    }
                }
            }
        }
...
void VectorIndicesMgr::checkPartCanBuildVI(MergeTreeDataPartPtr data_part, const VIDescription & vec_desc)
{
    ...
    if (data_part->segments_mgr->getSegmentStatus(vec_desc.name) == AIDB::SegmentStatus::CANCELLED)
    ...
    if (data_part->segments_mgr->getSegmentStatus(vec_desc.name) == AIDB::SegmentStatus::BUILT)
        throw Exception(
    ...
    bool slow_mode)
{
    ...
    ctx->index_build_memory_lock = std::make_unique<AIDB::VIBuildMemoryUsageHelper>();
                                  ...
                                  vec_index_desc = ctx->vec_index_desc]() -> bool
    {
        ...
        if (vi_status->getStatus() == AIDB::SegmentStatus::CANCELLED)
            return true;
...
AIDB::SegmentBuiltStatus VectorIndicesMgr::buildVI(const VIContextPtr ctx)
{
    try
    {
        LOG_INFO(log, "Build vector index {} for part {} start.", ctx->vector_index_name, ctx->source_part->name);
        if (ctx->source_part->segments_mgr->getSegmentStatus(ctx->vector_index_name) == AIDB::SegmentStatus::BUILT)
        {
            LOG_WARNING(log, "Vector index has been created successfully and does not need to be built again. it's a bug.");
            return AIDB::SegmentBuiltStatus{AIDB::SegmentBuiltStatus::BUILD_SKIPPED};
        }

        MergeTreeDataPartPtr future_part = data.getActiveContainingPart(ctx->source_part->name);
        if (future_part)
        {
            if (future_part->segments_mgr->getSegmentStatus(ctx->vector_index_name) == AIDB::SegmentStatus::BUILT)
            {
                LOG_WARNING(
                    log,
        ...
        ctx->vi_status->setStatus(AIDB::SegmentStatus::BUILDING);
        ctx->watch.restart();

        AIDB::SegmentBuiltStatus status = buildVIForOnePart(ctx);

        if (status.getStatus() == AIDB::SegmentBuiltStatus::SUCCESS)
            LOG_INFO(
                log,
    ...
    catch (Exception & e)
    {
        ...
        return AIDB::SegmentBuiltStatus{AIDB::SegmentBuiltStatus::BUILD_FAIL, e.code(), e.message()};
    }
}

AIDB::SegmentBuiltStatus VectorIndicesMgr::buildVIForOnePart(VIContextPtr ctx)
{
    if (ctx->build_cancel_callback())
        return AIDB::SegmentBuiltStatus{AIDB::SegmentBuiltStatus::BUILD_CANCEL};
    ...
    if (part_storage == nullptr)
        return AIDB::SegmentBuiltStatus{AIDB::SegmentBuiltStatus::BUILD_FAIL};
    ...
    if (disk->exists(ctx->vector_tmp_relative_path))
    {
        ...
        if (disk->exists(ctx->vector_tmp_relative_path + "/" + index_checksum_file))
        {
            ...
            return AIDB::SegmentBuiltStatus{AIDB::SegmentBuiltStatus::SUCCESS};
        }
    ...
    catch (Exception & e)
    {
        ...
        return AIDB::SegmentBuiltStatus{AIDB::SegmentBuiltStatus::BUILD_RETRY};
    }
...
AIDB::SegmentBuiltStatus VectorIndicesMgr::TryMoveVIFiles(const VIContextPtr ctx)
{
