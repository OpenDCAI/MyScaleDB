...
void SegmentsMgr::validateSegmentsInLock(const std::unique_lock<std::shared_mutex> & /*lock*/)
{
    ...
    for (auto [vi_name, segment] : segments)
    {
        ...
        segment->removeCache();
        segments_to_rebuild.emplace_back(vi_name, segment->getVIDescription());
    }

    for (const auto& [vi_name, vi_desc] : segments_to_rebuild)
    {
        segments.erase(vi_name);
        segments[vi_name] = createSegment(vi_desc, data_part.shared_from_this());
    }
}
